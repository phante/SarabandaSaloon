 ...
 */
package com.phante.sarabandasaloon.view.simbols;
 ...
 * @author deltedes
 */
public class ButtonSimbol extends StackPane {
    ...
    public Map<ButtonStatus, SVGPath> simbols = new HashMap<>();
    ...
    public ButtonSimbol () {
        ...
        for (ButtonStatus status: ButtonStatus.values()) {
            ...
            simbol.setVisible(status == ButtonStatus.ENABLED);
        ...
        simbols.get(ButtonStatus.ENABLED).setContent(ENABLED);
        simbols.get(ButtonStatus.ENABLED).setFill(Color.GOLD);
        simbols.get(ButtonStatus.PRESSED).setContent(PRESSED);
        simbols.get(ButtonStatus.PRESSED).setFill(Color.GREEN);
        simbols.get(ButtonStatus.ERROR).setContent(ERROR);
        simbols.get(ButtonStatus.ERROR).setFill(Color.RED);
        simbols.get(ButtonStatus.DISABLED).setContent(DISABLED);
        simbols.get(ButtonStatus.DISABLED).setFill(Color.GREY);
        
        /*
        heightProperty().addListener(listener -> {
        simbols.values().stream().forEach((simbol) -> {
        simbol.scaleXProperty().setValue(getHeight()/simbol.maxHeight(Double.MAX_VALUE));
        });
        });
    ...
    /*
    public void setValue(String newStatus) {
        setValue(ButtonStatus.valueOf(newStatus));
    }
    */
    
    public void setValue(ButtonStatus newStatus) {
        for (ButtonStatus status: ButtonStatus.values()) {
